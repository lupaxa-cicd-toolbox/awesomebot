...
PACKAGE_NAME='awesome_bot'
BASE_COMMAND='awesome_bot'
INSTALLED_CHECK=("${BASE_COMMAND}" --version)
INSTALL_COMMAND=(gem install --quiet "${PACKAGE_NAME}")
TEST_COMMAND=("${BASE_COMMAND}")
EXTRA_PARAMETERS=()
...
handle_non_standard_parameters()
{
    ...
    if [[ -n "${FLAGS-}" ]]; then
        if [[ "${FLAGS}" == 'default' ]]; then
            extra_params="${DEFAULT_FLAG_GROUP}"
        else
            extra_params=${FLAGS}
        fi
    else
        extra_params="${DEFAULT_FLAGS}"
    fi
    parameters=true
    PARAMETERS_BUFFER+=" Flags: ${bold_cyan_text}${extra_params}${reset}
"

    if [[ -n "${WHITELIST-}" ]]; then
        extra_params="${extra_params} --white-list ${WHITELIST//[[:blank:]]/}"
        PARAMETERS_BUFFER+=" Whitelist: ${bold_cyan_text}${WHITELIST}${reset}
"
    fi
